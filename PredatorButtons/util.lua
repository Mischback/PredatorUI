...
local proxifyButton = function(btn)
    ...
    if not util.buttonFuncProxy[bname.."SetSize"] then
        util.buttonFuncProxy[bname.."SetSize"] = btn.SetSize
        btn.SetSize = util.noop
    end
    if not util.buttonFuncProxy[bname.."SetPoint"] then
        util.buttonFuncProxy[bname.."SetPoint"] = btn.SetPoint
        btn.SetPoint = util.noop
    end
end
...
local scaleButton = function(btn, sizeX, sizeY)
    util.buttonFuncProxy[btn:GetName().."SetSize"](btn, sizeX, sizeY)
end
...
local moveButton = function(btn, pos)
    btn:ClearAllPoints()
    util.buttonFuncProxy[btn:GetName().."SetPoint"](btn, unpack(pos))
end
...
local fetchButtonsFromBar = function(key, parent)
    ...
    for i = 1, settings.static.NumButtons[key] do
        ...
        if i <= PredatorButtonsSettings[key].buttons then
            ...
            btn:ClearAllPoints()
            util.buttonFuncProxy[btn:GetName().."SetPoint"](btn, "BOTTOMRIGHT", UIParent, "TOPLEFT", -5, 5)
        end
    end
end
